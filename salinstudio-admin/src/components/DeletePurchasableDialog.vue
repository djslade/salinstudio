...
const submit = async () => {
  try {
    ...
    await refreshIfUnauthorized(async () => {
      await deleteRequest(`/purchasable/${storeItem.id}`, {
        accessToken: true,
      });
    });
    setStepEnd();
        ...
        <div class="flex justify-center items-center flex-col gap-6">
          <div class="flex flex-col gap-3 items-center">
            <i
              ...
              "
            />
            <span class="text-lg">Successfully deleted store item.</span>
          </div>
  ...
  <div v-if="isStart()" class="flex flex-col w-full gap-6">
    <div class="flex w-full justify-center">
      <Image alt="Image" preview class="w-full">
        ...
        <template #image>
          <img
            :src="storeItem.art.image.thumbUrl"
            :alt="storeItem.art.titleEn"
            class="aspect-video object-cover w-full rounded-lg"
          />
        </template>
        <template #original="slotProps">
          <img
            :src="storeItem.art.image.desktopUrl"
            :alt="storeItem.art.titleEn"
    ...
    <span class="text-center"
      >Are you sure you want to delete this store item?</span
    >
