...
import { FileUpload, Card, Button, Image, useToast, OrderList } from "primevue";
...
import type { Purchasable, Image as ImageType } from "../types/data";
...
const currentImages = ref<ImageType[]>();

const { data } = useQuery({
  queryKey: [`purchasable.${route.params.id}`],
  queryFn: async () => {
    const res = await getRequest<{ purchasable: Purchasable }>(
      `/purchasable/${route.params.id}`,
      { accessToken: true },
    );
    return res.purchasable;
  },
...
const handleCreateStoreItem = async (evt: FormSubmitEvent) => {
  ...
  try {
    ...
    saveFormValues({
      ...
      infoEn: evt.values.infoEn,
      infoFi: evt.values.infoFi,
...
const handleImageDelete = (id: string) => {
  if (!currentImages.value) return;
  currentImages.value = currentImages.value.filter((image) => image.id !== id);
};

const onPageLoad = (value?: Purchasable) => {
  if (value && !pageReady.value) {
    saveFormValues({
      artId: value.art.id,
      ...
      infoEn: value.infoEn,
      infoFi: value.infoFi,
      techniqueEn: value.techniqueEn,
      techniqueFi: value.techniqueFi,
      height: value.height,
      width: value.width,
      ...
      isFramed: value.isFramed,
      isFeatured: value.isFeatured,
      isOnSale: value.isOnSale,
    });
    currentImages.value = value.images;
...
watch(data, (data) => onPageLoad(data));
</script>
            ...
            <div class="w-full flex-1 flex flex-col gap-6">
              ...
              >
                <template #option="{ option }">
                  ...
                  <div class="">
                    <Button
                      v-if="data && data.art.image.id === option.id"
                      v-tooltip.bottom="'You can\'t delete this image'"
                      icon="pi pi-trash"
                      rounded
                      severity="danger"
                      raised
                      disabled
                      @click="() => handleImageDelete(option.id)"
                    />
                    <Button
                      v-else
                      v-tooltip.bottom="'You can\'t delete this image'"
